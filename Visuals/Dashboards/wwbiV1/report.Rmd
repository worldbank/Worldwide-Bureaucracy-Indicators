...
# Add graphs for wage bill section
  ...
  if (wb_gdp & wb_exp) {
    ggarrange(wb_gdp_plot, wb_exp_plot, ncol=2, common.legend = TRUE, legend="bottom")
...
`r if(publicemp){"\\vspace{15pt}\\begin{sectionbox}\\center\\Large\\color{sectiontitle}\\textbf{Public employment}\\end{sectionbox}"}`
```{r, eval = publicemp}
...
if (wb_emp & wb_age & wb_gender & wb_edu) {
  print(ggarrange(wb_gender_plot, wb_edu_plot, ncol=2, common.legend = TRUE, legend="bottom"))
  print(ggarrange(wb_emp_plot, wb_age_plot, ncol=2, common.legend = TRUE, legend="bottom"))
} else if (wb_emp & !wb_age & !wb_gender & !wb_edu) {
  wb_emp_plot
}  else if (wb_emp & wb_age & wb_gender & !wb_edu) {
  ggarrange(wb_emp_plot, wb_age_plot, ncol=2, common.legend = TRUE, legend="bottom")
   wb_gender_plot
} else if (wb_emp & !wb_age & wb_gender & wb_edu) {
  ggarrange(wb_emp_plot, wb_gender_plot, ncol=2, common.legend = TRUE, legend="bottom")
   wb_edu_plot
}else if (wb_emp & wb_age & !wb_gender & wb_edu) {
 ggarrange(wb_emp_plot, wb_age_plot, ncol=2, common.legend = TRUE, legend="bottom")
   wb_edu_plot
...
```
`r if(wb_premium){"\\vspace{15pt}\\begin{sectionbox}\\center\\Large\\color{sectiontitle}\\textbf{Wage premium}\\end{sectionbox}"}`

```{r, eval = wb_premium, fig.width= 6.5, fig.height= 4,fig.show='hold',fig.align='center', warning = F}
