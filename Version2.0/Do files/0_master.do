...
*Regional databases are used for processing the data to ensure managable file size 

 global bases "BRA EAP ECA EUSILC GLD HI-NON_OECD HI-OECD IDN LAC MNA REV6 SAR SSA"
...
*   REQUIRES: "${Input}ALL IN ONE_7_`var'.dta"	
*   CREATES : "${Input}ALL IN ONE_7_`var'_s2000.dta"	
...
*	REQUIRES: "${Input}ALL IN ONE_7_`var'_s2000.dta"	
...
*	        :"${nput}ALL IN ONE_7_`var'_s2000.dta"
...
*-------------------------------------------
*Part 1e
*Industry - Constructs industry dummies for industrial decomposition

*	REQUIRES:"${Data}LatestI2D2_`var'.dta"
*	        :"${Output}WWBI Version 2.0 Index.dta"
*	CREATES	:"${Temp}Industry_`var'.dta"
	
	do "${Do}1e_industry.do"
...
*-------------------------------------------
*Part 2g
*Summary G - Indicators on public sector emoployment ratios, by industries 

*	REQUIRES:"${Temp}Industry_`var'.dta"
*	CREATES:"${Temp}SummaryG_`var'.dta"
	
	do "${Do}2g_SummaryG.do"

*-------------------------------------------
*Part 2h
*Summary H - Indicators on wage premium and gender wage differentials, by industries 

*	REQUIRES:"${Temp}Industry_`var'.dta"
*	CREATES:"${Temp}SummaryH_`var'.dta"
	
	do "${Do}2h_SummaryH.do"
...
*-------------------------------------------
*Part 3a
*Merging - Merge all summary statistics together from part 2a-2h
...
*			 "${Temp}summaryG_`var'.dta"
*			 "${Temp}summaryH_`var'.dta"
*	CREATES	:"${Output}Summary_`var'.dta"

    do "${Do}3a_merge.do"

*Append all summary datasets from different region together

use"${Output}summary_BRA.dta", clear

global bases2 "EAP ECA EUSILC GLD HI-NON_OECD HI-OECD IDN LAC MNA REV6 SAR SSA"

foreach var of global bases2 {
	append using  "${Output}summary_`var'.dta"
}

save "${Output}Summary_WWBI.dta",replace
...
*	CREATES	:"${Output}WWBI_v2.0_clean.dta"
