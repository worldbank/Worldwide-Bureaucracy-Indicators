...
### 2.1: Using only Formal Employment vs GDP per capita

The marginal cost of adding a toggle between different measures of employment is high and bug-prone, so I just created this graph using only `Formal Employoment`. If we like this setup I can try to implement the y-axis variable toggle.

Note: at this point we're trying to figure out if we should add the by-region best fit lines.

#### 2.1: no by-region lines

```{r}
p7
```

#### 2.2: by-region lines, with dots
```{r}
p7.2
```
...
### 3.1: Comparing all three measures of public employment colored by region
...
####3.2 But here's a version using only the three most recent years for each country
```{r}
p5.1
```


### Readily-available color schemes
...
### 4 : Heatmap / Traffic light map 

The only thing to note here is that there are too many countries to have the plot render all of the text on the y axis. Alternatives I can think of include: <br>
- simply not including the y axis labels (the mouse over does include country labels...)
- make the graph longer so that all the names render (see next figure)
...
### 5: Wage bill vs log GDP per capita
There are two different measures for wage bill as percent of GDP, so I made each sepeartely and tried putting them on the same graph
